...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case UPDATE_PRODUCT:
      ...
      let newCategories_x_products = state.categores_x_products.filter(
        (el) => id !== el.product_id
      );
      categories.forEach((cat) =>
        newCategories_x_products.push({
          product_id: id,
          category: cat,
        })
      );
    ...
    case DELETE_CATEGORY:
      return {
        ...state,
        categories: [
          ...state.categories.filter((cat) => cat.name !== action.payload),
        ],
      };
    case ADD_USER:
      return {
    ...
    case DELETE_USER:
      return {
        ...
        ...state,
        all_users: [
          ...state.all_users.filter((user) => user.id !== action.payload),
        ],
      };
    ...
    case DELETE_PRODUCT_CART:
      return {
        ...
        getcart: state.getcart.filter(
          (prod) =>
            prod.order_id !== action.payload.orderId &&
            prod.product_id !== action.payload.productId
        ),
